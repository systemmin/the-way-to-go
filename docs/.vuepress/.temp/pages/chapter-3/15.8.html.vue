<template><div><h1 id="_15-8-精巧的多功能网页服务器" tabindex="-1"><a class="header-anchor" href="#_15-8-精巧的多功能网页服务器"><span>15.8 精巧的多功能网页服务器</span></a></h1>
<p>为进一步深入理解 <code v-pre>http</code> 包以及如何构建网页服务器功能，让我们来学习和体会下面的例子：先列出代码，然后给出不同功能的实现方法，程序输出显示在表格中。</p>
...
<tbody>
<tr>
<td><code v-pre>Logger()</code></td>
<td>http://localhost:12345/ （根）</td>
<td>oops</td>
</tr>
</tbody>
</table>
<p><code v-pre>Logger()</code> 处理函数用 <code v-pre>w.WriteHeader(404)</code> 来输出 “404 Not Found”头部。</p>
<p>这项技术通常很有用，无论何时服务器执行代码产生错误，都可以应用类似这样的代码：</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>另外利用 <code v-pre>logger</code> 包的函数，针对每个请求在服务器端命令行打印日期、时间和 URL。</p>
<table>
<thead>
<tr>
<th>处理函数</th>
...
<tbody>
<tr>
<td><code v-pre>HelloServer()</code></td>
<td>http://localhost:12345/go/hello</td>
<td>hello, world!</td>
</tr>
</tbody>
</table>
<p>包 <code v-pre>expvar</code> 可以创建（Int，Float 和 String 类型）变量，并将它们发布为公共变量。它会在 HTTP URL <code v-pre>/debug/vars</code> 上以 JSON 格式公布。通常它被用于服务器操作计数。<code v-pre>helloRequests</code> 就是这样一个 <code v-pre>int64</code> 变量，该处理函数对其加 1，然后写入“hello world!”到浏览器。</p>
<table>
<thead>
<tr>
<th>处理函数</th>
...
<tbody>
<tr>
<td><code v-pre>Counter()</code></td>
<td>http://localhost:12345/counter</td>
<td>counter = 1</td>
</tr>
<tr>
<td><code v-pre>Counter()</code></td>
...
<p>计数器对象 <code v-pre>ctr</code> 有一个 <code v-pre>String()</code> 方法，所以它实现了 <code v-pre>expvar.Var</code> 接口。这使其可以被发布，尽管它是一个结构体。<code v-pre>ServeHTTP()</code> 函数使 <code v-pre>ctr</code> 成为处理器，因为它的签名正确实现了 <code v-pre>http.Handler</code> 接口。</p>
<table>
<thead>
<tr>
<th>处理函数</th>
...
<tbody>
<tr>
<td><code v-pre>FileServer()</code></td>
<td>http://localhost:12345/go/ggg.html</td>
<td>404 page not found</td>
</tr>
</tbody>
</table>
<p><code v-pre>FileServer(root FileSystem) Handler</code> 返回一个处理器，它以 <code v-pre>root</code> 作为根，用文件系统的内容响应 HTTP 请求。要获得操作系统的文件系统，用 <code v-pre>http.Dir</code>，例如：</p>
...
<tbody>
<tr>
<td><code v-pre>FlagServer()</code></td>
<td>http://localhost:12345/flags</td>
<td>Flags: boolean = true root = /home/rsc</td>
</tr>
</tbody>
</table>
<p>该处理函数使用了 <code v-pre>flag</code> 包。<code v-pre>VisitAll()</code> 函数迭代所有的标签 (flag)，打印它们的名称、值和默认值（当不同于“值”时）。</p>
<table>
<thead>
<tr>
<th>处理函数</th>
...
<tbody>
<tr>
<td><code v-pre>ArgServer()</code></td>
<td>http://localhost:12345/args</td>
<td>./elaborated_webserver.exe</td>
</tr>
</tbody>
</table>
<p>该处理函数迭代 <code v-pre>os.Args</code> 以打印出所有的命令行参数。如果没有指定则只有程序名称（可执行程序的路径）会被打印出来。</p>
<table>
<thead>
<tr>
<th>处理函数</th>
...
<tbody>
<tr>
<td><code v-pre>Channel()</code></td>
<td>http://localhost:12345/chan</td>
...
<p>每当有新请求到达，通道的 <code v-pre>ServeHTTP()</code> 方法从通道获取下一个整数并显示。由此可见，网页服务器可以从通道中获取要发送的响应，它可以由另一个函数产生（甚至是客户端）。下面的代码片段正是一个这样的处理函数，但会在 30 秒后超时：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">func</span> <span class="token function">ChanResponse</span><span class="token punctuation">(</span>w http<span class="token punctuation">.</span>ResponseWriter<span class="token punctuation">,</span> req <span class="token operator">*</span>http<span class="token punctuation">.</span>Request<span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
<tbody>
<tr>
<td><code v-pre>DateServer()</code></td>
<td>http://localhost:12345/date</td>
<td>显示当前时间（由于是调用 /bin/date，仅在 Unix 下有效）</td>
</tr>
</tbody>
</table>
<p>可能的输出：<code v-pre>Thu Sep 8 12:41:09 CEST 2011</code>。</p>
<p><code v-pre>os.Pipe()</code> 返回一对相关联的 <code v-pre>File</code>，从 <code v-pre>r</code> 读取数据，返回已读取的字节数来自于 <code v-pre>w</code> 的写入。函数返回这两个文件和错误，如果有的话：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">func</span> <span class="token function">Pipe</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">(</span>r <span class="token operator">*</span>File<span class="token punctuation">,</span> w <span class="token operator">*</span>File<span class="token punctuation">,</span> err <span class="token builtin">error</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><h2 id="链接" tabindex="-1"><a class="header-anchor" href="#链接"><span>链接</span></a></h2>
<ul>
<li><RouteLink to="/chapter-3/directory.html">目录</RouteLink></li>
<li>上一节：<RouteLink to="/chapter-3/15.7.html">探索 template 包</RouteLink></li>
<li>下一节：<RouteLink to="/chapter-3/15.9.html">用 rpc 实现远程过程调用</RouteLink></li>
</ul>
</div></template>
