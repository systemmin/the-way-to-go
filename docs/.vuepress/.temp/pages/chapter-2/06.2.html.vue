<template><div><h1 id="_6-2-函数参数与返回值" tabindex="-1"><a class="header-anchor" href="#_6-2-函数参数与返回值"><span>6.2 函数参数与返回值</span></a></h1>
<p>函数能够接收参数供自己使用，也可以返回零个或多个值（我们通常把返回多个值称为返回一组值）。相比与 C、C++、Java 和 C#，多值返回是 Go 的一大特性，为我们判断一个函数是否正常执行（参考 <RouteLink to="/chapter-2/05.2.html">第 5.2 节</RouteLink>）提供了方便。</p>
<p>我们通过 <code v-pre>return</code> 关键字返回一组值。事实上，任何一个有返回值（单个或多个）的函数都必须以 <code v-pre>return</code> 或 <code v-pre>panic</code>（参考 <RouteLink to="/chapter-2/13.0.html">第 13 章</RouteLink>）结尾。</p>
<p>在函数块里面，<code v-pre>return</code> 之后的语句都不会执行。如果一个函数需要返回值，那么这个函数里面的每一个代码分支 (code-path) 都要有 <code v-pre>return</code> 语句。</p>
<p>问题 6.1：下面的函数将不会被编译，为什么呢？大家可以试着纠正过来。</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">func</span> <span class="token punctuation">(</span>st <span class="token operator">*</span>Stack<span class="token punctuation">)</span> <span class="token function">Pop</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token builtin">int</span> <span class="token punctuation">{</span></span>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>函数定义时，它的形参一般是有名字的，不过我们也可以定义没有形参名的函数，只有相应的形参类型，就像这样：<code v-pre>func f(int, int, float64)</code>。</p>
<p>没有参数的函数通常被称为 <strong>niladic</strong> 函数 (niladic function)，就像 <code v-pre>main.main()</code>。</p>
<h2 id="_6-2-1-按值传递-call-by-value-按引用传递-call-by-reference" tabindex="-1"><a class="header-anchor" href="#_6-2-1-按值传递-call-by-value-按引用传递-call-by-reference"><span>6.2.1 按值传递 (call by value) 按引用传递 (call by reference)</span></a></h2>
<p>Go 默认使用按值传递来传递参数，也就是传递参数的副本。函数接收参数副本之后，在使用变量的过程中可能对副本的值进行更改，但不会影响到原来的变量，比如 <code v-pre>Function(arg1)</code>。</p>
<p>如果你希望函数可以直接修改参数的值，而不是对参数的副本进行操作，你需要将参数的地址（变量名前面添加 <code v-pre>&amp;</code> 符号，比如 <code v-pre>&amp;variable</code>）传递给函数，这就是按引用传递，比如 <code v-pre>Function(&amp;arg1)</code>，此时传递给函数的是一个指针。如果传递给函数的是一个指针，指针的值（一个地址）会被复制，但指针的值所指向的地址上的值不会被复制；我们可以通过这个指针的值来修改这个值所指向的地址上的值。（<strong>译者注：指针也是变量类型，有自己的地址和值，通常指针的值指向一个变量的地址。所以，按引用传递也是按值传递。</strong>）</p>
...
<p>如下，simple_function.go 里的 <code v-pre>MultiPly3Nums</code> 函数带有三个形参，分别是 <code v-pre>a</code>、<code v-pre>b</code>、<code v-pre>c</code>，还有一个 <code v-pre>int</code> 类型的返回值（被注释的代码具有和未注释部分同样的功能，只是多引入了一个本地变量）：</p>
...
<h2 id="_6-2-2-命名的返回值-named-return-variables" tabindex="-1"><a class="header-anchor" href="#_6-2-2-命名的返回值-named-return-variables"><span>6.2.2 命名的返回值 (named return variables)</span></a></h2>
<p>如下 multiple_return.go 里的函数带有一个 <code v-pre>int</code> 参数，返回两个 <code v-pre>int</code> 值；其中一个函数的返回值在函数调用时就已经被赋予了一个初始零值。</p>
<p><code v-pre>getX2AndX3</code> 与 <code v-pre>getX2AndX3_2</code> 两个函数演示了如何使用非命名返回值与命名返回值的特性。当需要返回多个非命名返回值时，需要使用 <code v-pre>()</code> 把它们括起来，比如 <code v-pre>(int, int)</code>。</p>
<p>命名返回值作为结果形参 (result parameters) 被初始化为相应类型的零值，当需要返回的时候，我们只需要一条简单的不带参数的 <code v-pre>return</code> 语句。需要注意的是，即使只有一个命名返回值，也需要使用 <code v-pre>()</code> 括起来（参考<RouteLink to="/chapter-2/06.6.html">第 6.6 节</RouteLink> 的 <a href="./examples/chapter_6/fibonacci.go">fibonacci.go</a> 函数）。</p>
...
<p>提示：</p>
<p>虽然 <code v-pre>return</code> 或 <code v-pre>return var</code> 都是可以的，但是 <code v-pre>return var = expression</code>（表达式） 会引发一个编译错误：</p>
<p><code v-pre>syntax error: unexpected =, expecting semicolon or newline or }</code>。</p>
<p>即使函数使用了命名返回值，你依旧可以无视它而返回明确的值。</p>
<p>任何一个非命名返回值（使用非命名返回值是很糟的编程习惯）在 <code v-pre>return</code> 语句里面都要明确指出包含返回值的变量或是一个可计算的值（就像上面警告所指出的那样）。</p>
...
<p>编写一个名字为 <code v-pre>MySqrt()</code> 的函数，计算一个 <code v-pre>float64</code> 类型浮点数的平方根，如果参数是一个负数的话将返回一个错误。编写两个版本，一个是非命名返回值，一个是命名返回值。</p>
<h2 id="_6-2-3-空白符-blank-identifier" tabindex="-1"><a class="header-anchor" href="#_6-2-3-空白符-blank-identifier"><span>6.2.3 空白符 (blank identifier)</span></a></h2>
<p>空白符用来匹配一些不需要的值，然后丢弃掉，下面的 blank_identifier.go 就是很好的例子。</p>
<p><code v-pre>ThreeValues</code> 是拥有三个返回值的不需要任何参数的函数，在下面的例子中，我们将第一个与第三个返回值赋给了 <code v-pre>i1</code> 与 <code v-pre>f1</code>。第二个返回值赋给了空白符 <code v-pre>_</code>，然后自动丢弃掉。</p>
...
<h2 id="_6-2-4-改变外部变量-outside-variable" tabindex="-1"><a class="header-anchor" href="#_6-2-4-改变外部变量-outside-variable"><span>6.2.4 改变外部变量 (outside variable)</span></a></h2>
<p>传递指针给函数不但可以节省内存（因为没有复制变量的值），而且赋予了函数直接修改外部变量的能力，所以被修改的变量不再需要使用 <code v-pre>return</code> 返回。如下的例子，<code v-pre>reply</code> 是一个指向 <code v-pre>int</code> 变量的指针，通过这个指针，我们在函数内修改了这个 <code v-pre>int</code> 变量的数值。</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这仅仅是个指导性的例子，当需要在函数内改变一个占用内存比较大的变量时，性能优势就更加明显了。然而，如果不小心使用的话，传递一个指针很容易引发一些不确定的事，所以，我们要十分小心那些可以改变外部变量的函数，在必要时，需要添加注释以便其他人能够更加清楚的知道函数里面到底发生了什么。</p>
</div></template>
