<template><div><h1 id="_7-1-声明和初始化" tabindex="-1"><a class="header-anchor" href="#_7-1-声明和初始化"><span>7.1 声明和初始化</span></a></h1>
<h2 id="_7-1-1-概念" tabindex="-1"><a class="header-anchor" href="#_7-1-1-概念"><span>7.1.1 概念</span></a></h2>
<p>数组是具有相同 <strong>唯一类型</strong> 的一组已编号且长度固定的数据项序列（这是一种同构的数据结构）；这种类型可以是任意的原始类型例如整型、字符串或者自定义类型。数组长度必须是一个常量表达式，并且必须是一个非负整数。数组长度也是数组类型的一部分，所以 <code v-pre>[5]int</code> 和 <code v-pre>[10]int</code> 是属于不同类型的。数组的编译时值初始化是按照数组顺序完成的（如下）。</p>
<div class="hint-container tip">
<p class="hint-container-title">Tips</p>
<p>如果我们想让数组元素类型为任意类型的话可以使用空接口作为类型（参考 <RouteLink to="/chapter-2/11.9.html">第 11 章</RouteLink>）。当使用值时我们必须先做一个类型判断（参考 <RouteLink to="/chapter-2/11.3.html">第 11 章</RouteLink>）。</p>
</div>
<p>数组元素可以通过 <strong>索引</strong>（位置）来读取（或者修改），索引从 <code v-pre>0</code> 开始，第一个元素索引为 <code v-pre>0</code>，第二个索引为 <code v-pre>1</code>，以此类推（数组以 0 开始在所有类 C 语言中是相似的）。元素的数目（也称为长度或者数组大小）必须是固定的并且在声明该数组时就给出（编译时需要知道数组长度以便分配内存）；数组长度最大为 2GB。</p>
<p>声明的格式是：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">var</span> identifier <span class="token punctuation">[</span><span class="token builtin">len</span><span class="token punctuation">]</span><span class="token keyword">type</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>例如：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">var</span> arr1 <span class="token punctuation">[</span><span class="token number">5</span><span class="token punctuation">]</span><span class="token builtin">int</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>在内存中的结构是：<img src="/images/7.1_fig7.1.png" alt=""></p>
<p>每个元素是一个整型值，当声明数组时所有的元素都会被自动初始化为默认值 0。</p>
<p><code v-pre>arr1</code> 的长度是 5，索引范围从 <code v-pre>0</code> 到 <code v-pre>len(arr1)-1</code>。</p>
<p>第一个元素是 <code v-pre>arr1[0]</code>，第三个元素是 <code v-pre>arr1[2]</code>；总体来说索引 <code v-pre>i</code> 代表的元素是 <code v-pre>arr1[i]</code>，最后一个元素是 <code v-pre>arr1[len(arr1)-1]</code>。</p>
<p>对索引项为 <code v-pre>i</code> 的数组元素赋值可以这么操作：<code v-pre>arr[i] = value</code>，所以数组是 <strong>可变的</strong>。</p>
<p>只有有效的索引可以被使用，当使用等于或者大于 <code v-pre>len(arr1)</code> 的索引时：如果编译器可以检测到，会给出索引超限的提示信息；如果检测不到的话编译会通过而运行时会 <code v-pre>panic()</code>:（参考<RouteLink to="/chapter-2/13.0.html">第 13 章</RouteLink>）</p>
<pre><code>runtime error: index out of range
</code></pre>
<p>由于索引的存在，遍历数组的方法自然就是使用 <code v-pre>for</code> 结构：</p>
<ul>
<li>通过 <code v-pre>for</code> 初始化数组项</li>
<li>通过 <code v-pre>for</code> 打印数组元素</li>
<li>通过 <code v-pre>for</code> 依次处理元素</li>
</ul>
...
<p><code v-pre>for</code> 循环中的条件非常重要：<code v-pre>i &lt; len(arr1)</code>，如果写成 <code v-pre>i &lt;= len(arr1)</code> 的话会产生越界错误。</p>
<p>IDIOM:</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>也可以使用 for-range 的生成方式：</p>
<p>IDIOM:</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">for</span> i<span class="token punctuation">,</span><span class="token boolean">_</span><span class="token operator">:=</span> <span class="token keyword">range</span> arr1 <span class="token punctuation">{</span></span>
<span class="line"><span class="token operator">...</span></span>
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这里 <code v-pre>i</code> 也是数组的索引。当然这两种 <code v-pre>for</code> 结构对于切片（<code v-pre>slices</code>）（参考 <RouteLink to="/chapter-2/07.2.html">第 7 章</RouteLink>）来说也同样适用。</p>
<p><strong>问题 7.1</strong> 下面代码段的输出是什么？</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line">a <span class="token operator">:=</span> <span class="token punctuation">[</span><span class="token operator">...</span><span class="token punctuation">]</span><span class="token builtin">string</span><span class="token punctuation">{</span><span class="token string">"a"</span><span class="token punctuation">,</span> <span class="token string">"b"</span><span class="token punctuation">,</span> <span class="token string">"c"</span><span class="token punctuation">,</span> <span class="token string">"d"</span><span class="token punctuation">}</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Go 语言中的数组是一种 <strong>值类型</strong>（不像 C/C++ 中是指向首元素的指针），所以可以通过 <code v-pre>new()</code> 来创建： <code v-pre>var arr1 = new([5]int)</code>。</p>
<p>那么这种方式和 <code v-pre>var arr2 [5]int</code> 的区别是什么呢？<code v-pre>arr1</code> 的类型是 <code v-pre>*[5]int</code>，而 <code v-pre>arr2</code> 的类型是 <code v-pre>[5]int</code>。</p>
<p>这样的结果就是当把一个数组赋值给另一个时，需要再做一次数组内存的拷贝操作。例如：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line">arr2 <span class="token operator">:=</span> <span class="token operator">*</span>arr1</span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><p>这样两个数组就有了不同的值，在赋值后修改 <code v-pre>arr2</code> 不会对 <code v-pre>arr1</code> 生效。</p>
<p>所以在函数中数组作为参数传入时，如 <code v-pre>func1(arr2)</code>，会产生一次数组拷贝，<code v-pre>func1()</code> 方法不会修改原始的数组 <code v-pre>arr2</code>。</p>
<p>如果你想修改原数组，那么 <code v-pre>arr2</code> 必须通过 <code v-pre>&amp;</code> 操作符以引用方式传过来，例如 <code v-pre>func1(&amp;arr2)</code>，下面是一个例子：</p>
...
<p>另一种方法就是生成数组切片并将其传递给函数（详见<RouteLink to="/chapter-2/07.1.html">第 7.1.4 节</RouteLink>）。</p>
...
<p>在<RouteLink to="/chapter-2/06.6.html">第 6.6 节</RouteLink> 我们看到了一个递归计算 Fibonacci 数值的方法。但是通过数组我们可以更快的计算出 Fibonacci 数。完成该方法并打印出前 50 个 Fibonacci 数字。</p>
<h2 id="_7-1-2-数组常量" tabindex="-1"><a class="header-anchor" href="#_7-1-2-数组常量"><span>7.1.2 数组常量</span></a></h2>
<p>如果数组值已经提前知道了，那么可以通过 <strong>数组常量</strong> 的方法来初始化数组，而不用依次使用 <code v-pre>[]=</code> 方法（所有的组成元素都有相同的常量语法）。</p>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>第一种变化：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">var</span> arrAge <span class="token operator">=</span> <span class="token punctuation">[</span><span class="token number">5</span><span class="token punctuation">]</span><span class="token builtin">int</span><span class="token punctuation">{</span><span class="token number">18</span><span class="token punctuation">,</span> <span class="token number">20</span><span class="token punctuation">,</span> <span class="token number">15</span><span class="token punctuation">,</span> <span class="token number">22</span><span class="token punctuation">,</span> <span class="token number">16</span><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>注意 <code v-pre>[5]int</code> 可以从左边起开始忽略：<code v-pre>[10]int {1, 2, 3}</code> :这是一个有 10 个元素的数组，除了前三个元素外其他元素都为 <code v-pre>0</code>。</p>
<p>第二种变化：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">var</span> arrLazy <span class="token operator">=</span> <span class="token punctuation">[</span><span class="token operator">...</span><span class="token punctuation">]</span><span class="token builtin">int</span><span class="token punctuation">{</span><span class="token number">5</span><span class="token punctuation">,</span> <span class="token number">6</span><span class="token punctuation">,</span> <span class="token number">7</span><span class="token punctuation">,</span> <span class="token number">8</span><span class="token punctuation">,</span> <span class="token number">22</span><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p><code v-pre>...</code> 同样可以忽略，从技术上说它们其实变成了切片。</p>
<p>第三种变化：<code v-pre>key: value 语法</code></p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">var</span> arrKeyValue <span class="token operator">=</span> <span class="token punctuation">[</span><span class="token number">5</span><span class="token punctuation">]</span><span class="token builtin">string</span><span class="token punctuation">{</span><span class="token number">3</span><span class="token punctuation">:</span> <span class="token string">"Chris"</span><span class="token punctuation">,</span> <span class="token number">4</span><span class="token punctuation">:</span> <span class="token string">"Ron"</span><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>只有索引 3 和 4 被赋予实际的值，其他元素都被设置为空的字符串，所以输出结果为：</p>
...
<p>在这里数组长度同样可以写成 <code v-pre>...</code>。</p>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>输出结果：</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre v-pre><code><span class="line">&amp;[0 0 0]</span>
<span class="line">&amp;[1 1 1]</span>
<span class="line">&amp;[2 4 8]</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>几何点（或者数学向量）是一个使用数组的经典例子。为了简化代码通常使用一个别名：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">type</span> Vector3D <span class="token punctuation">[</span><span class="token number">3</span><span class="token punctuation">]</span><span class="token builtin">float32</span></span>
<span class="line"><span class="token keyword">var</span> vec Vector3D</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_7-1-3-多维数组" tabindex="-1"><a class="header-anchor" href="#_7-1-3-多维数组"><span>7.1.3 多维数组</span></a></h2>
<p>数组通常是一维的，但是可以用来组装成多维数组，例如：<code v-pre>[3][5]int</code>，<code v-pre>[2][2][2]float64</code>。</p>
<p>内部数组总是长度相同的。Go 语言的多维数组是矩形式的（唯一的例外是切片的数组，参见<RouteLink to="/chapter-2/07.2.html">第 7.2.5 节</RouteLink>。</p>
...
<p>但这在 Go 中并不常用，通常使用切片（参考 <RouteLink to="/chapter-2/07.2.html">第 7.2 节</RouteLink>）。</p>
</div></template>
