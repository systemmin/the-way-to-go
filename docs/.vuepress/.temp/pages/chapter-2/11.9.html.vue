<template><div><h1 id="_11-9-空接口" tabindex="-1"><a class="header-anchor" href="#_11-9-空接口"><span>11.9 空接口</span></a></h1>
<h2 id="_11-9-1-概念" tabindex="-1"><a class="header-anchor" href="#_11-9-1-概念"><span>11.9.1 概念</span></a></h2>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>任何其他类型都实现了空接口（它不仅仅像 <code v-pre>Java/C#</code> 中 <code v-pre>Object</code> 引用类型），<code v-pre>any</code> 或 <code v-pre>Any</code> 是空接口一个很好的别名或缩写。</p>
<p>空接口类似 <code v-pre>Java/C#</code> 中所有类的基类： <code v-pre>Object</code> 类，二者的目标也很相近。</p>
<p>可以给一个空接口类型的变量 <code v-pre>var val interface {}</code> 赋任何类型的值。</p>
...
<p>在上面的例子中，接口变量 <code v-pre>val</code> 被依次赋予一个 <code v-pre>int</code>，<code v-pre>string</code> 和 <code v-pre>Person</code> 实例的值，然后使用 <code v-pre>type-switch</code> 来测试它的实际类型。每个 <code v-pre>interface {}</code> 变量在内存中占据两个字长：一个用来存储它包含的类型，另一个用来存储它包含的数据或者指向数据的指针。</p>
<p>示例 <a href="examples/chapter_11/emptyint_switch.go">emptyint_switch.go</a> 说明了空接口在 <code v-pre>type-switch</code> 中联合 <code v-pre>lambda</code> 函数的用法：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">package</span> main</span>
<span class="line"></span>
<span class="line"><span class="token keyword">import</span> <span class="token string">"fmt"</span></span>
<span class="line"></span>
...
<p><strong>练习 11.9</strong> <a href="exercises/chapter_11/simple_interface3.go">simple_interface3.go</a>：</p>
<p>继续练习 11.2，在它中添加一个 <code v-pre>gI()</code> 函数，它不再接受 <code v-pre>Simpler</code> 类型的参数，而是接受一个空接口参数。然后通过类型断言判断参数是否是 <code v-pre>Simpler</code> 类型。最后在 <code v-pre>main</code> 使用 <code v-pre>gI()</code> 取代 <code v-pre>fI()</code> 函数并调用它。确保你的代码足够安全。</p>
<h2 id="_11-9-2-构建通用类型或包含不同类型变量的数组" tabindex="-1"><a class="header-anchor" href="#_11-9-2-构建通用类型或包含不同类型变量的数组"><span>11.9.2 构建通用类型或包含不同类型变量的数组</span></a></h2>
<p>在 <RouteLink to="/chapter-2/07.6.html">7.6.6</RouteLink> 中我们看到了能被搜索和排序的 <code v-pre>int</code> 数组、<code v-pre>float</code> 数组以及 <code v-pre>string</code> 数组，那么对于其他类型的数组呢，是不是我们必须得自己编程实现它们？</p>
<p>现在我们知道该怎么做了，就是通过使用空接口。让我们给空接口定一个别名类型 <code v-pre>Element</code>：<code v-pre>type Element interface{}</code></p>
<p>然后定义一个容器类型的结构体 <code v-pre>Vector</code>，它包含一个 <code v-pre>Element</code> 类型元素的切片：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">type</span> Vector <span class="token keyword">struct</span> <span class="token punctuation">{</span></span>
<span class="line">	a <span class="token punctuation">[</span><span class="token punctuation">]</span>Element</span>
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>Vector</code> 里能放任何类型的变量，因为任何类型都实现了空接口，实际上 <code v-pre>Vector</code> 里放的每个元素可以是不同类型的变量。我们为它定义一个 <code v-pre>At()</code> 方法用于返回第 <code v-pre>i</code> 个元素：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">func</span> <span class="token punctuation">(</span>p <span class="token operator">*</span>Vector<span class="token punctuation">)</span> <span class="token function">At</span><span class="token punctuation">(</span>i <span class="token builtin">int</span><span class="token punctuation">)</span> Element <span class="token punctuation">{</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>再定一个 <code v-pre>Set()</code> 方法用于设置第 <code v-pre>i</code> 个元素的值：</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>Vector</code> 中存储的所有元素都是 <code v-pre>Element</code> 类型，要得到它们的原始类型（unboxing：拆箱）需要用到类型断言。TODO：The compiler rejects assertions guaranteed to fail，类型断言总是在运行时才执行，因此它会产生运行时错误。</p>
<p><strong>练习 11.10</strong> <a href="exercises/chapter_11/min_interface.go">min_interface.go</a> / <a href="exercises/chapter_11/minmain.go">minmain.go</a>：</p>
<p>仿照 11.7 中开发的 <code v-pre>Sorter</code> 接口，创建一个 <code v-pre>Miner</code> 接口并实现一些必要的操作。函数 <code v-pre>Min()</code> 接受一个 <code v-pre>Miner</code> 类型变量的集合，然后计算并返回集合中最小的元素。</p>
<h2 id="_11-9-3-复制数据切片至空接口切片" tabindex="-1"><a class="header-anchor" href="#_11-9-3-复制数据切片至空接口切片"><span>11.9.3 复制数据切片至空接口切片</span></a></h2>
<p>假设你有一个 <code v-pre>myType</code> 类型的数据切片，你想将切片中的数据复制到一个空接口切片中，类似：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">var</span> dataSlice <span class="token punctuation">[</span><span class="token punctuation">]</span>myType <span class="token operator">=</span> <span class="token function">FuncReturnSlice</span><span class="token punctuation">(</span><span class="token punctuation">)</span></span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><p>可惜不能这么做，编译时会出错：<code v-pre>cannot use dataSlice (type []myType) as type []interface { } in assignment</code>。</p>
<p>原因是它们俩在内存中的布局是不一样的（参考 <a href="https://github.com/golang/go/wiki/InterfaceSlice" target="_blank" rel="noopener noreferrer">Go wiki</a>）。</p>
<p>必须使用 <code v-pre>for-range</code> 语句来一个一个显式地赋值：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">var</span> dataSlice <span class="token punctuation">[</span><span class="token punctuation">]</span>myType <span class="token operator">=</span> <span class="token function">FuncReturnSlice</span><span class="token punctuation">(</span><span class="token punctuation">)</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_11-9-4-通用类型的节点数据结构" tabindex="-1"><a class="header-anchor" href="#_11-9-4-通用类型的节点数据结构"><span>11.9.4 通用类型的节点数据结构</span></a></h2>
<p>在 <RouteLink to="/chapter-2/10.1.html">10.1</RouteLink> 中我们遇到了诸如列表和树这样的数据结构，在它们的定义中使用了一种叫节点的递归结构体类型，节点包含一个某种类型的数据字段。现在可以使用空接口作为数据字段的类型，这样我们就能写出通用的代码。下面是实现一个二叉树的部分代码：通用定义、用于创建空节点的 <code v-pre>NewNode</code> 方法，及设置数据的 <code v-pre>SetData</code> 方法。</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_11-9-5-接口到接口" tabindex="-1"><a class="header-anchor" href="#_11-9-5-接口到接口"><span>11.9.5 接口到接口</span></a></h2>
<p>一个接口的值可以赋值给另一个接口变量，只要底层类型实现了必要的方法。这个转换是在运行时进行检查的，转换失败会导致一个运行时错误：这是 <code v-pre>Go</code> 语言动态的一面，可以拿它和 <code v-pre>Ruby</code> 和 <code v-pre>Python</code> 这些动态语言相比较。</p>
<p>假定：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">var</span> ai AbsInterface <span class="token comment">// declares method Abs()</span></span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>那么下面的语句和类型断言是合法的：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line">empty <span class="token operator">=</span> pp                <span class="token comment">// everything satisfies empty</span></span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>下面是函数调用的一个例子：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go" data-title="go"><pre v-pre><code><span class="line"><span class="token keyword">type</span> myPrintInterface <span class="token keyword">interface</span> <span class="token punctuation">{</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>x</code> 转换为 <code v-pre>myPrintInterface</code> 类型是完全动态的：只要 <code v-pre>x</code> 的底层类型（动态类型）定义了 <code v-pre>print</code> 方法这个调用就可以正常运行（译注：若 <code v-pre>x</code> 的底层类型未定义 <code v-pre>print</code> 方法，此处类型断言会导致 <code v-pre>panic</code>，最佳实践应该为 <code v-pre>if mpi, ok := x.(myPrintInterface); ok { mpi.print() }</code>，参考 <RouteLink to="/chapter-2/11.3.html">11.3</RouteLink> 章节）。</p>
</div></template>
